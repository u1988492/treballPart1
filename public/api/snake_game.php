...
// Conectar también a la base de usuarios para leer preferencias
$db_users_path = __DIR__ . '/../../private/users.db';
...
function get_user_preferences($user_id) {
    global $db_users;
    $stmt = $db_users->prepare("SELECT display_name, preferred_color, preferred_controls FROM users WHERE user_id = ?");
    ...
    if (!$user) {
        return [
            ...
            'preferred_controls' => 'wasd'
        ];
    }
    
    return [
        'display_name' => $user['display_name'] ?? null,
        'preferred_color' => $user['preferred_color'] ?? '#00FF00',
        'preferred_controls' => $user['preferred_controls'] ?? 'wasd'
    ];
}
...
try {
    ...
    if ($action === 'get_user_preferences') {
        $user_prefs = get_user_preferences($authenticated_user_id);
        echo json_encode($user_prefs);
        exit;
    }
    ...
    if ($action === 'create_lobby') {
        ...
        $user_prefs = get_user_preferences($authenticated_user_id);
        $player_name = $user_prefs['display_name'] ?? $authenticated_user_name;
        $player_color = $user_prefs['preferred_color'];
        ...
        echo json_encode([
            ...
            'player_id' => $player_id,
            'player_number' => 1,
            'preferred_controls' => $user_prefs['preferred_controls']
        ]);
        exit;
    }
    ...
    if ($action === 'join_game') {
        ...
        $stmt = $db->prepare("SELECT player1_snake, player2_id, player1_id FROM game_state WHERE game_id = ?");
        ...
        // Prevenir que el jugador se una a su propio juego
    ...
    if ($action === 'list_games') {
        $player_id = get_player_id();
        $stmt = $db->query("SELECT game_id, player1_id, player1_name, player1_color, created_at FROM game_state WHERE game_status='waiting'");
        $games = [];
        foreach ($stmt as $row) {
            $games[] = [
                'game_id' => $row['game_id'],
                ...
                'is_own_game' => ($row['player1_id'] === $player_id)
            ];
        }
    ...
    if ($action === 'cancel_game') {
        ...
        // Verificar que el juego existe y está en espera
        $stmt = $db->prepare("SELECT player1_id, game_status FROM game_state WHERE game_id = ?");
        $stmt->execute([$game_id]);
        $game = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        // Solo permitir cancelación si es el creador
        if ($game['player1_id'] !== $player_id) {
            throw new Exception('Only the game creator can cancel');
        }
        
        // Solo permitir cancelación si está en espera
        if ($game['game_status'] !== 'waiting') {
            throw new Exception('Can only cancel games in waiting state');
        }
        
        // Eliminar el juego
        $stmt_delete = $db->prepare("DELETE FROM game_state WHERE game_id = ?");
        $stmt_delete->execute([$game_id]);
        
        // También eliminar registros de latencia asociados
        $stmt_latency = $db->prepare("DELETE FROM player_latency WHERE game_id = ?");
        $stmt_latency->execute([$game_id]);
        
        echo json_encode(['success' => true, 'message' => 'Game cancelled']);
        exit;
    }
    
    // ==================== LEAVE GAME (abandonar partida activa) ====================
    if ($action === 'leave_game') {
        // sendBeacon envía datos como application/x-www-form-urlencoded en el body
        // Leer el body raw si $_POST está vacío
        ...
        $game_id = $_POST['game_id'] ?? '';
        // Obtener player_id desde la sesión autenticada
        $player_id = get_player_id();
        
        error_log("[LEAVE_GAME] Recibido: game_id=$game_id, player_id=$player_id (from session)");
        
        if (empty($game_id)) {
            error_log("[LEAVE_GAME] Error: Missing game_id");
        ...
        if (!$game) {
            error_log("[LEAVE_GAME] Error: Game not found - $game_id");
            throw new Exception('Game not found');
        }
        
        error_log("[LEAVE_GAME] Game players: player1_id={$game['player1_id']}, player2_id={$game['player2_id']}");
        
        // Verificar que el jugador pertenece al juego
        if ($game['player1_id'] !== $player_id && $game['player2_id'] !== $player_id) {
            error_log("[LEAVE_GAME] Error: Player $player_id not in game $game_id");
            throw new Exception('You are not in this game');
        }
        
        // Solo terminar si el juego está activo
        if ($game['game_status'] === 'playing') {
            // Determinar quién gana (el que NO abandonó)
            $winner_id = ($game['player1_id'] === $player_id) ? $game['player2_id'] : $game['player1_id'];
            
            // Actualizar estado del juego a terminado
            $stmt_update = $db->prepare("
                ...
                WHERE game_id = ?
            ");
            $stmt_update->execute([$winner_id, microtime(true), $game_id]);
            
            error_log("[LEAVE_GAME] ✓ Jugador $player_id abandonó la partida $game_id. Ganador: $winner_id");
            
            echo json_encode(['success' => true, 'message' => 'Game ended due to player leaving']);
        } else if ($game['game_status'] === 'waiting') {
            // Si está esperando, simplemente cancelar
            $stmt_delete = $db->prepare("DELETE FROM game_state WHERE game_id = ?");
            $stmt_delete->execute([$game_id]);
            
            error_log("[LEAVE_GAME] ✓ Juego $game_id cancelado (estaba en espera)");
            
            echo json_encode(['success' => true, 'message' => 'Game cancelled']);
        } else {
            error_log("[LEAVE_GAME] Info: Juego $game_id ya estaba terminado");
            echo json_encode(['success' => true, 'message' => 'Game already finished']);
        }
